 ...
 */
public class SimpleLdapClientRepository<T> implements LdapRepository<T> {
	...
	public <S extends T> S save(S entity) {
		...
		return isNew(entity) ? ldap.create(entity) : ldap.update(entity);
	}
	...
	public Optional<T> findOne(LdapQuery ldapQuery) {
		...
		try {
			List<T> results = findAll(ldapQuery);
			if (results.size() != 1) {
