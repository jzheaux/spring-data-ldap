 ...
 * @since 2.6
 */
public class QuerydslLdapClientPredicateExecutor<T> implements ListQuerydslPredicateExecutor<T> {
	...
	/**
	 * Creates a new {@link QuerydslLdapClientPredicateExecutor}.
	 *
	...
	private QuerydslLdapClientQuery<T> queryFor(Predicate predicate) {
		return queryFor(predicate, it -> {

		});
	}

	private QuerydslLdapClientQuery<T> queryFor(Predicate predicate, Consumer<LdapQueryBuilder> queryBuilderConsumer) {
	...
	class FluentQuerydsl<R> implements FluentQuery.FetchableFluentQuery<R> {
		...
		public Stream<R> stream() {
			...
			return search(null, QuerydslLdapClientQuery::list).stream().map(conversionFunction);
		}
		...
		private List<T> findTop(int limit) {
			return search(limit, QuerydslLdapClientQuery::list);
		}

		private <S> S search(@Nullable Integer limit, Function<QuerydslLdapClientQuery<T>, S> searchFunction) {

			QuerydslLdapClientQuery<T> q = queryFor(predicate, query -> {
