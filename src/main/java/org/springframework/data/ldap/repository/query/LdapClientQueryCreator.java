...
class LdapClientQueryCreator extends AbstractQueryCreator<Consumer<LdapQueryBuilder>, ContainerCriteria> {
	...
	/**
	 * Constructs a new {@link LdapClientQueryCreator}.
	 *
	 ...
	 */
	LdapClientQueryCreator(PartTree tree, Class<?> entityType, LdapMapperClient ldap,
						   LdapParameterAccessor parameterAccessor, List<String> inputProperties) {

		super(tree, parameterAccessor);

		Assert.notNull(entityType, "Entity type must not be null");
		Assert.notNull(ldap, "ObjectDirectoryMapper must not be null");

		this.entityType = entityType;
		this.ldap = ldap.map(entityType);
	...
	private String getAttribute(Part part) {
		...
		return ldap.field(path.getSegment());
	}
	...
	@Override
	protected Consumer<LdapQueryBuilder> complete(ContainerCriteria criteria, Sort sort) {
		return (builder) -> builder.base(criteria.base())
